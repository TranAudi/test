...
@Component({
  selector: 'app-bo-mon-item',
  templateUrl: './bo-mon-item.component.html',
  styleUrls: ['./bo-mon-item.component.less']
})
export class BoMonItemComponent implements OnInit {
  @Input() type = 'add';
  @Input() item: any;
  ...
  constructor(
    ...
    this.form = this.fb.group({
      maBoMon: [null, [Validators.required]],
      tenBoMon: [null, [Validators.required]],
      soNhom: [null, [Validators.required]]
    });
  ...
  update(data: any, qdKt: any) {
    ...
    this.tittle = 'Cập nhật bộ môn';

    this.form.get('maBoMon')?.setValue(data.maBoMon);
    this.form.get('tenBoMon')?.setValue(data.boMon);
    this.form.get('soNhom')?.setValue(data.soNhom);
  }

  saveBoMonItem() {
    var res = {
      idBoMon: this.idBoMon,
      maBoMon: this.form.value.maBoMon,
      boMon: this.form.value.tenBoMon,
      soNhom: this.form.value.soNhom
    };

    if (this.isAdd) {
      this.boMonService.createBoMon(res).subscribe(res => {
        this.messageService.success('Lưu thành công');
        this.closeModalReloadData();
      });
    } else if (this.isEdit) {
      this.boMonService.updateBoMon(this.idBoMon, res).subscribe(res => {
        this.messageService.success('Lưu thành công');
        this.closeModalReloadData();
      });
    }
  ...
  saveSv() {
    ...
    if (this.selectedRows.length == 0) {
      this.messageService.error('Bạn phải chọn ít nhất 1 môn học để thêm');
    } else {
      this.handleCancelAddSv();
    }
  }
  initGridData() {
    this.grid.rowData = this.listBoMonItem;
  }
  getListSv() {
